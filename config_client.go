...
func (client *ConfigClient) getConfigInner(param vo.ConfigParam) (content string, err error) {
	...
	content, err = client.configProxy.GetConfigProxy(param, clientConfig.NamespaceId, clientConfig.AccessKey, clientConfig.SecretKey,clientConfig.Username,clientConfig.Password)
...
func (client *ConfigClient) searchConfigInnter(param vo.SearchConfigParm) (*model.ConfigPage, error) {
	...
	clientConfig, _ := client.GetClientConfig()
	configItems, err := client.configProxy.SearchConfigProxy(param, clientConfig.NamespaceId, clientConfig.AccessKey, clientConfig.SecretKey,clientConfig.Username,clientConfig.Password)
	if err != nil {
