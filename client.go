...
func NewNacosClient(nodes []string, group string, config constant.ClientConfig) (client *Client, err error) {
	...
	log.Info(fmt.Sprintf("endpoint=%s, namespace=%s, group=%s, accessKey=%s, secretKey=%s, openKMS=%d, regionId=%s,username=%s,password=%s", config.Endpoint, config.NamespaceId, group, config.AccessKey, config.SecretKey, config.OpenKMS, config.RegionId,config.Username,config.Password))
	...
	client = &Client{configClient, namingClient, group, config.NamespaceId, config.AccessKey, config.SecretKey,config.Username,config.Password, make(chan int, 10)}
